...
push to diarization_queue. No real ffmpeg work â€” for dev purposes every
...
def process_file(file_id, minio_url, filename):
    print(f"ðŸŽ­ [mock_convert] '{filename}' (file_id={file_id}) â€” skipping real "
          f"conversion, forwarding as-is")
    job = {"file_id": file_id, "minio_url": minio_url}
    r.lpush("diarization_queue", json.dumps(job))
    print(f"âœ… [mock_convert] pushed to diarization_queue")


def worker_loop():
    print("ðŸš€ [mock_convert] started, waiting for jobs...")
    while True:
        try:
            ...
            print(f"âš ï¸ [mock_convert] error: {e}")
            time.sleep(5)
