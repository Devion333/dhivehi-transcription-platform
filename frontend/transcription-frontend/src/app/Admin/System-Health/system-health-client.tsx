...
export function SystemHealthClient() {
  ...
  return (
    <PageContainer>
      <PageHeader title="System Health" description="Concise operational status without internal URLs, payloads, credentials, or worker identifiers." actions={<Button variant="outline" onClick={() => setReload((value) => value + 1)}><RefreshCcw className="h-4 w-4" /> Refresh</Button>} />
      ...
      {!loading && !error && (
        <div className="grid gap-4 lg:grid-cols-2">
          <Card><CardHeader><CardTitle>Core Services</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2">{services.map((item) => <HealthRow key={item.name} name={item.name} status={item.status} />)}</CardContent></Card>
          <Card><CardHeader><CardTitle>Workers</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2">{stages.map((stage) => <HealthRow key={stage} name={stage} status={jobs?.workers?.[stage]?.status ?? "unknown"} detail={`${jobs?.workers?.[stage]?.instances ?? 0} active`} />)}</CardContent></Card>
        </div>
      )}
    </PageContainer>
  );
